...
router.put("/:id", async function (req, res, next) {
  try {
    ...
    const alreadyPaid = currResult.rows[0].paid_date;
    ...
    if (!currPaidDate && paid) {
      //use the Now function of psql in the query directly
      paidDate = new Date();
    } else if (!paid) {
      paidDate = null;
    } else {
      //stop here as no need to change anything
      paidDate = currPaidDate;
    }

    const result = await db.query(
      //use NOW() to leverage database time when adding dates to the database
      `UPDATE invoices
